...
const Contact = () => {
  ...
  const fetchContent = async () => {
    try {
      ...
      const response = await apiClient.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/admin/content-settings/public`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await apiClient.post(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/contact`, formData);
  ...
  const email = content?.contactPage?.email || 'info@greendye.org';
