...
const VerifyCertificate = () => {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      const verificationUrl = `/verify/certificate/${encodeURIComponent(certificateId.trim())}`;
      
      const response = await apiClient.get(verificationUrl);
      
      setCertificate({ ...response.data.data, status: response.data.status });
  ...
  const getCredentialTitle = (credentialTitle) => {
    if (typeof credentialTitle === 'object' && credentialTitle !== null) {
      return credentialTitle.en || credentialTitle.ar || credentialTitle.fr || 'N/A';
    }
    return credentialTitle || 'N/A';
  };
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} sm={9}>
              <TextField
                ...
                placeholder="e.g., CERT-A1B2C3D4E5F6478890ABCDEF12345678"
                value={certificateId}
              ...
              <Grid container spacing={3}>
                ...
                {certificate.credentialTitle && (
                  <Grid item xs={12}>
                    ...
                    <Typography variant="body1" fontWeight="medium">
                      {getCredentialTitle(certificate.credentialTitle)}
                    </Typography>
                  </Grid>
                )}
