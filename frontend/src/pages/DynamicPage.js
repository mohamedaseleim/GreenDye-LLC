...
const DynamicPage = () => {
  ...
  useEffect(() => {
    ...
    if (page) {
      ...
      document.title = `${title} - GreenDye for Training and Consultancy`;
    ...
    return () => {
      document.title = 'GreenDye for Training and Consultancy';
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, language]);
  ...
  const fetchPage = async () => {
    try {
      setLoading(true);
      setError(false);
      const response = await apiClient.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/pages/${slug}`);
