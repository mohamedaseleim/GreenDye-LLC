...
const AdminTrainers = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const response = await adminService.getUsers({ role: 'individual_client', limit: 100 });
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4, mb: 4 }}>
      ...
      {/* Professional Metrics Dialog */}
      <Dialog open={openMetricsDialog} onClose={() => setOpenMetricsDialog(false)} maxWidth="md" fullWidth>
        <DialogTitle>Professional Metrics</DialogTitle>
        <DialogContent>
          {trainerMetrics && <Box sx={{ mt: 2 }}><Grid container spacing={3}>
            <Grid item xs={12} md={6}><Card><CardContent><Typography variant="h6" color="primary">Experience</Typography><Typography variant="h4">{trainerMetrics.professional?.experience || 0} years</Typography><Typography variant="body2">Accreditations: {trainerMetrics.professional?.accreditations || 0}</Typography></CardContent></Card></Grid>
            <Grid item xs={12} md={6}><Card><CardContent><Typography variant="h6" color="primary">Verification</Typography><Typography variant="h4">{trainerMetrics.professional?.verificationStatus || 'pending'}</Typography><Typography variant="body2">Professional profile review status</Typography></CardContent></Card></Grid>
            <Grid item xs={12} md={4}><Card><CardContent><Typography variant="h6">Total Earnings</Typography><Typography variant="h5">{trainerMetrics.financial?.totalEarnings || 0}</Typography></CardContent></Card></Grid>
            <Grid item xs={12} md={4}><Card><CardContent><Typography variant="h6">Pending Payout</Typography><Typography variant="h5">{trainerMetrics.financial?.pendingPayout || 0}</Typography></CardContent></Card></Grid>
            <Grid item xs={12} md={4}><Card><CardContent><Typography variant="h6">Paid Out</Typography><Typography variant="h5">{trainerMetrics.financial?.totalPaidOut || 0}</Typography></CardContent></Card></Grid>
          </Grid></Box>}
        </DialogContent><DialogActions><Button onClick={() => setOpenMetricsDialog(false)}>Close</Button></DialogActions>
      </Dialog>
