...
const sendEmail = async (options) => {
  try {
    ...
    const mailOptions = {
      from: `${process.env.FROM_NAME || 'GreenDye for Training and Consultancy'} <${process.env.FROM_EMAIL || process.env.SMTP_USER}>`,
      to: options.to,
...
const createEmailTemplate = (content, options = {}) => {
  const { title = 'GreenDye for Training and Consultancy', preheader = '' } = options;
  ...
  return `
    ...
    <body>
      ...
      <div class="container">
        <div class="header">
          <h1>GreenDye for Training and Consultancy</h1>
        </div>
        <div class="content">
          ${content}
        </div>
        <div class="footer">
          <p>&copy; ${new Date().getFullYear()} GreenDye for Training and Consultancy. All rights reserved.</p>
          <p>You are receiving this email because you are a member of GreenDye for Training and Consultancy.</p>
        </div>
      </div>
    </body>
    </html>
