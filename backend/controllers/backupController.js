...
const createDatabaseBackup = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    logger.error('backup.create.failed',{message:error.message});
    res.status(500).json({
      success: false,
...
const exportAllData = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    logger.error('backup.export.failed',{message:error.message});
    res.status(500).json({
      success: false,
...
const restoreDatabase = asyncHandler(async (req, res) => {
  if (process.env.NODE_ENV === 'production' && process.env.ALLOW_PRODUCTION_RESTORE !== 'true') {
    res.status(403);
    throw new Error('Production restore/import is disabled. Set ALLOW_PRODUCTION_RESTORE=true temporarily through the server environment after an independent backup.');
  }
  ...
  try {
    ...
    logger.error('backup.restore.failed',{message:error.message});
    
    // Clean up on error
    try {
      await fs.rm(extractPath, { recursive: true, force: true });
    } catch (cleanupError) {
      logger.warn('backup.cleanup.failed',{message:cleanupError.message});
    }
...
const listBackups = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    logger.error('backup.list.failed',{message:error.message});
    res.status(500).json({
      success: false,
